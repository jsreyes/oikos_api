...
type EspacioFisico struct {
	...
	Area             float32            `orm:"column(area);null"`
	ConexionInternet string             `orm:"column(conexion_internet);null"`
	ConexionTelefono string             `orm:"column(conexion_telefono);null"`
	Edificio         *Edificio          `orm:"column(edificio);rel(fk)"`
